...
class Vector2d():
    ...
    def Is_PointOnLine(p: tuple, line_p1: tuple, line_p2: tuple) -> bool:
        return Vector2d.distance_points(p, line_p1) + Vector2d.distance_points(p, line_p2) == Vector2d.distance_points(
            line_p1, line_p2)
...
class APF_VNS():
    ...
    def VNS(self, neighbourhood_names: list):
        ...
        while k != len(neighbourhood_names) and t != 70:
            ...
            local_SGs = self.bestImprovement(shaked_SGs,
                                             self.neighbourhood_selector(neighbourhood_names[k - 1], shaked_SGs))
    ...
    def shuffing(self, cur_SGs: list) -> list:
        ...
        while True:
            ...
            new_pos = (
            self.subgoals[ran_index][0] + cos * self.step_size, self.subgoals[ran_index][1] + sin * self.step_size)
        ...
        while (self.cur_iters < self.max_iters and (self.current_pos - self.goal).length > self.goal_threshold):
            ...
            if len(self.subgoals) != 0:
                ...
                if (len(self.path) >= 3 and (
                        Vector2d.Tuple_init(self.path[-2]) - self.current_pos).length < self.step_size / 100):
    ...
    def neighbourhood_obs_free(self, cur_SGs: list) -> list:
        ...
        for _ in range(8):
            ...
            for sg in cur_SGs:
                ...
                success = False
                t = 1
                while success == False and t < 1000:
                    ran_degree = random.randint(0, 360)
                    cos = math.cos(math.radians(ran_degree))
                    sin = math.sin(math.radians(ran_degree))
                    for ob in self.V_obstacle_list:
...
if __name__ == '__main__':
    ...
    APF1 = APF_VNS(start, goal, obstacle_List2, k_att, k_rep, rep_range, step_size, max_iters, goal_threshold, length,
                   num_sub, neighbour_name)
    ...
    subplot.plot([p[0] for p in APF1.path], [p[1] for p in APF1.path], linestyle='-', marker='o', color='g',
                 label="path")
    subplot.plot([p[0] for p in APF1.all_subgoals[-1]], [p[1] for p in APF1.all_subgoals[-1]], markersize=20,
                 marker='*', color='r', alpha=0.3, label="final subgoals")
    subplot.plot([p[0] for p in APF1.first_stuck], [p[1] for p in APF1.first_stuck], linestyle=':', markersize=15,
                 marker='*', color='g', label="first stuck subgoals", alpha=0.3)
    ...
    subplot2.plot([p[0] for p in APF1.first_stuck], [p[1] for p in APF1.first_stuck], linestyle='-', markersize=15,
                  marker='o', color='r', label="initial subgoals", alpha=0.3)
