...
class APF_DA():
    ...
    def U_att(self, pos):
        return (0.5 * self.k_att * ((pos - self.goal).length) ** 2) / self.temperature
    ...
    def U_rep(self, pos):
        ...
        for ob in self.V_obstacle_list:
            ...
            if (rep_F.length >= self.rep_range / self.temperature):  
                all_U += 0.5 * self.k_rep * (1 / (rep_F.length) - (1 / (self.rep_range / self.temperature))) ** 2
        return all_U
    ...
    def attractive_F(self):
        ...
        '''
        att = (self.goal - self.current_pos) * (self.k_att / self.temperature)
        return att
    ...
    def repulsion_F(self):
        ...
        for ob in self.V_obstacle_list:
            ...
            if (rep_F.length >= self.rep_range / self.temperature):  # 在斥力影响范围
                rep += Vector2d(rep_F.Unit_Vec[0], rep_F.Unit_Vec[1]) * self.k_rep * (
                    1.0 / rep_F.length - 1.0 / (self.rep_range / self.temperature)) / (rep_F.length ** 2)
        return rep
    ...
    def path_plan(self):
        ...
        # 此处加入逃逸算法 --------------------------------------
            if (len(self.path) >= 3 and (Vector2d(self.path[-2][0], self.path[-2][1]) - self.current_pos).length < self.step_size):
                ...
                while (self.temperature >= Tm or first):
                    first = False
                    ...
                    if (del_U <= 0):
                        ...
                        if (math.exp((-1 * del_U) / (self.temperature * 1)) > random.random()):
                            self.current_pos = new_pos
                            self.path.append([self.current_pos.deltaX, self.current_pos.deltaY])
                    if U_new <= U_cur:
                        need_cool = True
                    ...
                    self.temperature = self.temperature * r
                    print(self.temperature)
                need_cool = True
        # 逃逸算法end --------------------------------------
            self.cur_iters += 1
...
if __name__ == '__main__':
    ...
    APF1 = APF_DA(start, goal, obstacle_List2, k_att, k_rep, rep_range, step_size, max_iters, goal_threshold)
    APF1.path_plan()
